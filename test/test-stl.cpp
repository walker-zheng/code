...
int test_cref()
{
  ...
  std::vector<int> v_i(l.begin(), l.end());
  std::random_shuffle(v_i.begin(), v_i.end());
...
int test_partition()
{
  ...
  std::vector<int> v_1{0, 0, 3, 0, 2, 4, 5, 0, 7};
  std::stable_partition(v_1.begin(), v_1.end(), [](int n){return n>0;});
  for (int n : v_1) {
    std::cout << n << ' ';
  }
  std::cout << '\n';
